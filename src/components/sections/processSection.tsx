        ...
        <section id="processo" className="bg-N1 md:px-[12.5%] px-[26px] w-full h-fit min-h-[1065px] sm:min-h-[1280px] flex-col  sm:grid  sm:gap-[48px] pb-[62px]">
            ...
            <div className="grid absolute sm:relative sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 items-stretch content-between justify-items-stretch gap-x-11  gap-y-[52px] ">
                {processCardItem.map((item) => (
                    <div key={item.id} className={` max-sm:hidden max-w-[330px]  min-h-[280px] h-auto relative rounded-[8px] pt-[36px] pb-[26px]  px-10 text-center flex flex-col justify-between  items-center hover:ring-R5 shadow-[2px_2px_8px_rgba(0,0,0,0.4)] hover:shadow-[0px_1px_8px_#87240E] ring-[2px] hover:ring-[2.5px] duration-300  ease-out  transition-all text-black ring-[rgba(0,0,0,0.2)]  ${item.id == 9 ? "2xl:col-start-2 " : " "}`}><h1 className="absolute top-1 left-5 text-[40px] font-normal font-zodiak hidden text-N8">{item.id}</h1>
                        <Image src={item.icon} alt={item.title} className="h-[60px]" />
                        <h2 className="mt-[20px] font-montserrat font-bold text-[18px]">{item.title}</h2>
                        <p className="text-N5 text-[16px]">{item.descrição}</p>
                         <Link href="/processo" className="w-full flex justify-end pt-3">
                                <div>
            ...
            <div>
                {processCardItem.map((item, index) => (
                    ...
                    >
                        <Image src={item.icon} alt={item.title} className="size-[50px]  object-contain" />
